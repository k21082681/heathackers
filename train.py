...
FEATURE_ORDER = [
    ...
    'melt_fraction_x_lag1', 'melt_fraction_x_lag2', 'melt_fraction_x_lag3',
...
def compute_targets_if_missing(df, window_sec=60):
    ...
    if 'time_to_x95_min' not in df.columns or df['time_to_x95_min'].isna().all():
        ...
        for i in range(len(df)):
            future_x = df['melt_fraction_x'].iloc[i:i+window_rows]
...
def main():
    ...
    # Drop rows with NaN (from lags, look-ahead)
    df = df.dropna(subset=FEATURE_ORDER + TARGETS)
